...
export default function Feed() {
  ...
  // 👇 STATE MỚI: SẮP XẾP MỚI NHẤT / XU HƯỚNG 👇
  const [sortBy, setSortBy] = useState<'newest' | 'trending'>('newest');

  const { results, status, loadMore } = usePaginatedQuery(
    api.messages.getThreads,
    { channelId: activeChannelId || undefined, sortBy: sortBy }, // Truyền sortBy vào API
    { initialNumItems: 10 }
  );
  ...
  const searchServerPosts = useQuery(
    api.messages.searchMessages,
    searchTab === 'server' && debouncedSearch && targetWorkspaceId
      ? { search: debouncedSearch, ...(activeServerId ? { serverId: activeServerId } : { universityId: activeUniversityId }) }
      : "skip"
  );
    ...
    <div className="flex-1 flex flex-col h-full relative">
      ...
      <div className="h-14 bg-white border-b border-gray-200 flex items-center justify-between px-4 shrink-0 z-10 shadow-sm sticky top-0">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="font-bold text-gray-800 text-[16px] truncate">{activeChannelName || t('common.lobby')}</span>
        </div>
            ...
            {showNotifMenu && (
              <>
                ...
                <div className="absolute right-0 top-10 w-72 bg-white shadow-[0_8px_30px_rgb(0,0,0,0.12)] rounded-xl border border-gray-100 z-[50] py-2 animate-in fade-in slide-in-from-top-2">
                  <div className="px-4 py-2 font-extrabold text-[15px] text-center text-gray-800 border-b border-gray-100 mb-1 pb-3">{t('feed.notification_settings')}</div>
                  {!isLobby && (
                  ...
                  <button onClick={() => handleToggleServer('on')} className="w-full flex items-center justify-between px-4 py-3 text-[14px] text-gray-700 hover:bg-gray-50 transition-colors">
                    <span>{t('feed.turn_on_all_server')}</span><Bell className="w-4 h-4 text-gray-500" />
                  </button>
                  <button onClick={() => handleToggleServer('off')} className="w-full flex items-center justify-between px-4 py-3 text-[14px] text-red-500 hover:bg-red-50 transition-colors">
                    <span>{t('feed.turn_off_all_server')}</span><BellOff className="w-4 h-4 text-red-500" />
                  </button>
                  ...
                  <button onClick={() => { setShowNotifMenu(false); setShowCustomNotifModal(true); }} className="w-full flex items-center justify-between px-4 py-3 text-[14px] text-gray-700 hover:bg-gray-50 transition-colors">
                    <span>{t('feed.custom_per_channel')}</span><ChevronRight className="w-4 h-4 text-gray-400" />
                  </button>
                </div>
              </>
            )}
      ...
      <div id="main-scroll-area" ref={scrollRef} onScroll={handleScroll} className="flex-1 overflow-y-auto hidden-scrollbar p-4 pb-20">
        <div className="flex flex-col bg-white border border-gray-200 rounded-2xl overflow-hidden mb-10 shadow-sm">
            ...
            {/* 👇 BỘ LỌC NEWEST / TRENDING MƯỢN TỪ BẢN MOBILE 👇 */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 bg-gray-50/50">
              ...
              <div className="relative">
                <select
                  ...
                  className="appearance-none bg-white border border-gray-200 shadow-sm text-[13px] font-bold text-gray-700 py-1.5 pl-3 pr-8 rounded-lg outline-none cursor-pointer hover:bg-gray-50 transition-colors"
                >
                  <option value="newest">{t('feed.sort_newest')}</option>
                  <option value="trending">{t('feed.sort_trending')}</option>
                </select>
            ...
            {(status === "LoadingFirstPage" || status === "LoadingMore") && (
                <div className="flex justify-center items-center p-8"><div className="animate-spin h-6 w-6 border-2 border-gray-400 border-t-transparent rounded-full"></div></div>
            )}

            {status === "Exhausted" && results.length > 0 && (
                <p className="text-center text-gray-400 text-sm py-6 border-t border-gray-100">{t('profile_tabs.no_posts')}</p>
            )}
              ...
              {customChannels?.map((channel: any) => {
                return (
                  <div key={channel._id} className="flex items-center justify-between p-3 border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    <span className="text-[15px] font-medium text-gray-800 flex items-center gap-2"><Hash className="w-4 h-4 text-gray-400"/> {channel.name}</span>
                    <button onClick={() => handleToggleSpecificChannel(channel._id)} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${channel.isSubscribed ? 'bg-[#007AFF]' : 'bg-gray-200'}`}>
          ...
          <div className="bg-white w-full max-w-[750px] h-[75vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-4" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-4 border-b border-gray-100 shrink-0">
              <div className="bg-gray-100 rounded-xl flex items-center px-4 py-3">
                ...
                <input type="text" autoFocus placeholder={t('follow_list.search_placeholder')} value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="bg-transparent border-none outline-none flex-1 text-[16px] text-gray-800" />
            ...
            <div className="border-b border-gray-100 shrink-0">
              <div className="flex px-2 w-max min-w-full">
                <button onClick={() => setSearchTab('users')} className={`px-4 py-3 text-[14px] font-bold whitespace-nowrap shrink-0 border-b-2 transition-colors flex items-center gap-2 ${searchTab === 'users' ? 'border-[#007AFF] text-[#007AFF]' : 'border-transparent text-gray-500 hover:bg-gray-50'}`}><User className="w-4 h-4 shrink-0"/> {t('search.tab_users')}</button>
                <button onClick={() => setSearchTab('servers')} className={`px-4 py-3 text-[14px] font-bold whitespace-nowrap shrink-0 border-b-2 transition-colors flex items-center gap-2 ${searchTab === 'servers' ? 'border-[#007AFF] text-[#007AFF]' : 'border-transparent text-gray-500 hover:bg-gray-50'}`}><Server className="w-4 h-4 shrink-0"/> {t('search.tab_servers')}</button>
                <button onClick={() => setSearchTab('channel')} className={`px-4 py-3 text-[14px] font-bold whitespace-nowrap shrink-0 border-b-2 transition-colors flex items-center gap-2 ${searchTab === 'channel' ? 'border-[#007AFF] text-[#007AFF]' : 'border-transparent text-gray-500 hover:bg-gray-50'}`}><Hash className="w-4 h-4 shrink-0"/> {t('search.tab_channel_posts')}</button>
                {(activeServerId || activeUniversityId) && (
                  <button onClick={() => setSearchTab('server')} className={`px-4 py-3 text-[14px] font-bold whitespace-nowrap shrink-0 border-b-2 transition-colors flex items-center gap-2 ${searchTab === 'server' ? 'border-[#007AFF] text-[#007AFF]' : 'border-transparent text-gray-500 hover:bg-gray-50'}`}><MessageSquare className="w-4 h-4 shrink-0"/> {t('search.tab_server_posts')}</button>
                )}
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto bg-[#f2f3f5] p-4 hidden-scrollbar">
              {!debouncedSearch ? (
                <div className="h-full flex flex-col items-center justify-center text-gray-400"><Search className="w-12 h-12 mb-3 opacity-20" /><p>{t('follow_list.search_placeholder')}</p></div>
              ) : (
                <div className="space-y-3">
                  {searchTab === 'users' && searchUsersResult?.map((u: any) => (
                    <div key={u._id} onClick={() => { setShowSearchModal(false); router.push(`/profile/${u._id}`); }} className="bg-white p-3 rounded-xl flex items-center gap-3 cursor-pointer hover:bg-blue-50 transition-colors border border-gray-100 shadow-sm">
                      ...
                      <div><div className="font-bold text-gray-900 text-[15px]">{u.first_name || u.username}</div><div className="text-[13px] text-gray-500">@{u.username}</div></div>
                    </div>
                  ))}
                  ...
                  {searchTab === 'servers' && searchServersResult?.map((s: any) => (
                    <div key={s._id} className="bg-white p-3 rounded-xl flex items-center gap-3 border border-gray-100 shadow-sm">
                      ...
                      <div className="flex-1"><div className="font-bold text-gray-900 text-[15px]">{s.name}</div><div className="text-[13px] text-gray-500">{s.memberIds?.length || 0} {t('common.member')}</div></div>
                      <button onClick={() => handleServerClick(s)} className={`px-4 py-1.5 rounded-full text-sm font-bold ${s.isJoined ? 'bg-gray-100 text-gray-600' : 'bg-[#007AFF] hover:bg-blue-600 text-white'} transition-colors`}>{s.isJoined ? t('search.joined') : t('search.join')}</button>
                    </div>
                  ))}
                  ...
                  {(searchTab === 'channel' || searchTab === 'server') && (() => {
                    ...
                    return results.map((thread: any) => (
                      <div key={thread._id} className="rounded-xl overflow-hidden shadow-sm border border-gray-200 cursor-pointer hover:bg-gray-50 transition-colors" onClick={() => { setShowSearchModal(false); setSelectedThreadId(thread._id); }}>
                        <div className="pointer-events-none"><Thread thread={thread} /></div>
                      </div>
                    ));
                  })()}
                </div>
...
// =========================================================================
// COMPONENT: MÀN HÌNH CHI TIẾT BÀI VIẾT (Y HỆT BẢN MOBILE)
// =========================================================================
function PostDetailView({ messageId, onBack }: { messageId: string, onBack: () => void }) {
  ...
  if (mainThread === null) {
    return (
       <div className="flex-1 flex flex-col h-full bg-white relative">
          <div className="h-14 flex items-center justify-center px-4 border-b border-gray-200 relative shrink-0">
          ...
          <div className="flex-1 flex justify-center items-center"><p className="text-gray-500 font-medium">{t('thread.post_not_exist')}</p></div>
       </div>
    );
  }
  ...
  const handleReply = async () => {
     ...
     try {
        await addThread({ content: replyContent.trim(), threadId: mainThread._id, channelId: mainThread.channelId, serverId: mainThread.serverId, universityId: mainThread.universityId });
        setReplyContent("");
  ...
  return (
     <div className="flex-1 flex flex-col h-full bg-white relative animate-in fade-in slide-in-from-right-4 duration-200">
        <div className="h-14 flex items-center justify-center px-4 border-b border-gray-200 shrink-0 bg-white sticky top-0 relative">
           <button onClick={onBack} className="absolute left-2 p-2 hover:bg-gray-100 rounded-full transition-colors cursor-pointer" title={t('feed_layout.back')}><ArrowLeft className="w-6 h-6 text-gray-900" /></button>
           <span className="font-bold text-[18px] text-gray-900">{t('post_detail.title')}</span>
        </div>

        <div className="flex-1 overflow-y-auto hidden-scrollbar pb-24">
           <div className="border-b border-gray-100 shadow-sm"><Thread thread={mainThread as any} /></div>
        ...
        {mainThread.allowComments === false ? (
            <div className="absolute bottom-0 left-0 right-0 p-4 bg-gray-50 border-t border-gray-200 shadow-[0_-4px_10px_rgba(0,0,0,0.02)]">
               <p className="text-center text-gray-500 text-[14px]">{t('post_detail.comments_disabled_msg')}</p>
            </div>
        ) : (
            <div className="absolute bottom-0 left-0 right-0 p-3 bg-white border-t border-gray-200 shadow-[0_-4px_10px_rgba(0,0,0,0.02)]">
               <div className="flex items-center gap-3 bg-[#f2f3f5] rounded-full px-4 py-2">
                  ...
                  <input type="text" value={replyContent} onChange={(e) => setReplyContent(e.target.value)} placeholder={t('post_detail.add_reply_to', { name: authorName })} className="flex-1 bg-transparent border-none outline-none text-[15px] text-gray-800 placeholder-gray-500" onKeyDown={(e) => { if (e.key === 'Enter') handleReply(); }} />
