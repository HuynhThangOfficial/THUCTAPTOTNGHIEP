...
function SharedPostPreview({ postId }: { postId: string }) {
  ...
  if (thread === undefined) return <div className="p-3 bg-white rounded-lg border border-gray-200 w-full animate-pulse h-20"></div>;
  if (thread === null) return <div className="p-3 bg-white rounded-lg border border-gray-200 w-full text-center text-gray-500 italic text-xs">{t('chat.post_not_exist', {defaultValue: 'Bài viết không còn tồn tại'})}</div>;
  ...
  return (
    <div className="bg-white rounded-lg p-2.5 w-full max-w-[220px] border border-gray-200 shadow-sm cursor-pointer hover:bg-gray-50 transition-colors" onClick={() => window.open(`/post/${postId}`, '_blank')}>
      <div className="flex items-center gap-1.5 mb-1">
        <img src={displayAvatar} className="w-5 h-5 rounded-full object-cover" alt="avatar" />
        <span className="font-bold text-xs text-gray-900 truncate">{displayName}</span>
      </div>
      <p className="text-xs text-gray-800 line-clamp-2 mb-1.5">{thread.content}</p>
      <div className="flex items-center gap-3 pt-1 border-t border-gray-100 text-[10px] text-gray-500">
...
export default function ChatBox({ conversationId, onBack }: { conversationId: Id<"conversations">, onBack: () => void }) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#f2f3f5] relative overflow-hidden w-full">
      ...
      {/* --- HEADER CHAT (Đã fix lỗi mất nút Back và chỉnh nhỏ lại) --- */}
      <div className="h-14 border-b border-gray-200 bg-white flex items-center justify-between px-2 shrink-0 shadow-sm z-20">
        <div className="flex items-center gap-2 overflow-hidden">
          ...
          <div className="flex flex-col min-w-0">
            ...
            <p className="text-[11px] text-green-600 truncate">{otherUser.isOnline ? 'Đang hoạt động' : 'Ngoại tuyến'}</p>
          </div>
        </div>
      </div>
          ...
          {isPinnedExpanded ? (
            <div className="p-2">
              <div className="flex justify-between items-center mb-1 px-1">
                <span className="text-xs font-bold text-gray-800">{pinnedMessages.length} ghim</span>
              ...
              <div className="max-h-32 overflow-y-auto hidden-scrollbar">
                {[...pinnedMessages].reverse().map((pm: any) => (
                  <div key={pm._id} className="flex items-center gap-2 p-1.5 hover:bg-gray-50 cursor-pointer border-b border-gray-50">
                    ...
                    <span className="text-xs text-gray-700 truncate">{pm.imageUrl ? '[Hình ảnh]' : pm.content}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="flex items-center justify-between p-2 cursor-pointer hover:bg-gray-50" onClick={() => pinnedMessages.length > 1 ? setIsPinnedExpanded(true) : null}>
              <div className="flex items-center gap-2 overflow-hidden">
                ...
                <div className="flex flex-col truncate">
                  <span className="text-[10px] text-blue-600 font-bold leading-tight">Đã ghim</span>
                  <span className="text-xs text-gray-700 truncate">{pinnedMessages[pinnedMessages.length - 1].imageUrl ? '[Hình ảnh]' : pinnedMessages[pinnedMessages.length - 1].content}</span>
                </div>
              </div>
      ...
      {/* --- DANH SÁCH TIN NHẮN (Bóp khoảng cách để gọn hơn) --- */}
      <div className="flex-1 overflow-y-auto hidden-scrollbar p-3 space-y-2">
        {filteredMessages.map((msg: any, index: number) => {
          ...
          if (msg.isSystem) {
            ...
            if (msg.content.startsWith('SYS_PINNED|')) sysText = `${msg.content.split('|')[1]} đã ghim.`;
            else if (msg.content.startsWith('SYS_UNPINNED|')) sysText = `${msg.content.split('|')[1]} đã bỏ ghim.`;
          ...
          return (
            <div key={msg._id} className="flex flex-col">
              ...
              <div className={`flex gap-2 group relative ${isMe ? 'flex-row-reverse' : 'flex-row'} items-end ${hasReactions ? 'mb-3' : (isLastInGroup ? 'mb-1.5' : 'mb-0.5')}`}>
                ...
                {/* Tăng max-w để bóng chat không bị ép quá hẹp */}
                <div className={`flex flex-col max-w-[85%] ${isMe ? 'items-end' : 'items-start'} relative`}>
                  ...
                  <div className="flex items-center relative">
                    {!msg.isDeleted && (
                      <div className={`hidden group-hover:flex items-center absolute top-1/2 -translate-y-1/2 bg-white border border-gray-200 shadow-sm rounded-lg px-0.5 z-10 ${isMe ? 'right-full mr-1' : 'left-full ml-1'}`}>
                        <button onClick={() => setReplyingTo(msg)} className="p-1 text-gray-400 hover:text-blue-500 rounded"><Reply className="w-3.5 h-3.5" /></button>
                        <button onClick={() => togglePin({ messageId: msg._id, conversationId })} className="p-1 text-gray-400 hover:text-orange-500 rounded"><Pin className="w-3.5 h-3.5" /></button>
                      </div>
                    )}
                      ...
                      {repliedMsg && !msg.isDeleted && (
                        <div className={`mb-1 p-1.5 rounded-lg border-l-2 ${isMe ? 'bg-white/20 border-white' : 'bg-gray-100 border-[#007aff]'}`}>
                          <p className={`text-[11px] line-clamp-2 leading-tight ${isMe ? 'text-gray-100' : 'text-gray-500'}`}>
                            {repliedMsg.isDeleted ? 'Tin nhắn đã thu hồi' : (repliedMsg.imageUrl ? '[Hình ảnh]' : repliedMsg.content)}
                          </p>
                        </div>
                      )}
      ...
      {/* --- KHUNG NHẬP TIN NHẮN (Thu gọn vừa khít 300px) --- */}
        ...
        {replyingTo && (
          <div className="flex items-center justify-between px-3 py-1.5 bg-blue-50/50 border-b border-blue-100">
            <div className="flex flex-col overflow-hidden mr-2">
              <span className="text-[10px] font-bold text-blue-600 flex items-center gap-1"><Reply className="w-2.5 h-2.5" /> Trả lời</span>
              <span className="text-xs text-gray-600 truncate">{replyingTo.isDeleted ? 'Tin nhắn đã thu hồi' : (replyingTo.imageUrl ? '[Hình ảnh]' : replyingTo.content)}</span>
            </div>
        ...
        <div className="p-2 flex items-center gap-1.5">
          ...
          <div className="flex-1 flex items-center bg-gray-50 border border-gray-200 rounded-full px-3 py-1.5 focus-within:border-blue-400 transition-all min-w-0">
            <input 
              ...
              placeholder={isUploading ? 'Đang gửi ảnh...' : "Aa..."}
