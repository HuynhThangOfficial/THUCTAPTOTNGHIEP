...
import InviteFriendsModal from './InviteFriendsModal'; // 👈 IMPORT MODAL MỚI
...
export default function ServerSettingsModal({ onClose, workspace }: Props) {
  ...
  const [showInviteModal, setShowInviteModal] = useState(false); // 👈 THÊM STATE NÀY
  ...
  return (
    <>
        ...
        <div className="bg-[#f2f2f7] w-full max-w-[420px] rounded-2xl shadow-2xl flex flex-col animate-in zoom-in-95 duration-200 overflow-hidden min-h-[550px]" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-5">
            <div className="flex flex-col items-center mb-2">
              <div className="w-20 h-20 rounded-full bg-[#1c1c1e] flex items-center justify-center mb-3 overflow-hidden border border-gray-200 shadow-sm">
                {imagePreview ? (
                  <img src={imagePreview} alt="Server Avatar" className="w-full h-full object-cover" />
                ) : (
                  <span className="text-2xl font-bold text-white uppercase">{workspace.name.charAt(0)}</span>
                )}
              </div>
              <input type="file" accept="image/*" className="hidden" ref={fileInputRef} onChange={handleImageSelect} />
              <button onClick={() => fileInputRef.current?.click()} className="bg-[#e5e5ea] hover:bg-[#d1d1d6] text-black text-[13px] font-bold py-2 px-5 rounded-full transition-colors">
                {t('server.change_avatar')}
              </button>
            </div>
            ...
            {/* SỬA NÚT NÀY: MỞ MODAL MỜI BẠN BÈ */}
      ...
      {/* RENDER MODAL MỜI BẠN BÈ TẠI ĐÂY */}
