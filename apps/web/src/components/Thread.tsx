...
export default function Thread({ thread, isNested = false }: Props) {
  ...
  return (
    <div className={`bg-white ${isNested ? 'pt-3 pb-1' : 'p-4 border-b border-gray-200'} hover:bg-gray-50 transition-colors relative cursor-default`}>
        ...
        <div className="flex items-center gap-3 w-full min-w-0">
          ...
          <div className="flex-1 min-w-0 pr-2">
            <div className="flex items-center gap-2">
              <h4 onClick={handleGoToProfile} className={`font-bold text-gray-800 ${isNested ? 'text-[14px]' : 'text-[15px]'} truncate ${!isAnonymous ? 'cursor-pointer hover:underline' : ''}`}>
                 {displayName}
              </h4>
          ...
          {showOptions && (
            <>
              ...
              <div className="absolute right-0 mt-1 w-56 bg-white rounded-xl shadow-[0_4px_20px_rgba(0,0,0,0.1)] border border-gray-100 z-[50] py-1.5 overflow-hidden animate-in fade-in zoom-in-95 duration-100">
                {isOwner ? (
                  <>
                    <button onClick={(e) => { e.stopPropagation(); setIsEditing(true); setShowOptions(false); }} className="w-full flex items-center px-4 py-2.5 text-[14px] text-gray-700 hover:bg-gray-50 transition-colors"><Edit2 className="w-4 h-4 mr-3 text-gray-400" /> {t('thread.opt_edit')}</button>
                    <button onClick={(e) => { e.stopPropagation(); setShowHistoryModal(true); setShowOptions(false); }} className="w-full flex items-center px-4 py-2.5 text-[14px] text-gray-700 hover:bg-gray-50 transition-colors"><History className="w-4 h-4 mr-3 text-gray-400" /> {t('thread.opt_history')}</button>
                    <div className="h-px bg-gray-100 my-1"></div>
                    <button onClick={(e) => { e.stopPropagation(); handleDelete(); }} className="w-full flex items-center px-4 py-2.5 text-[14px] text-red-600 hover:bg-red-50 font-medium transition-colors"><Trash2 className="w-4 h-4 mr-3 text-red-500" /> {t('thread.opt_delete')}</button>
                  </>
                ) : (
                  <>
                    <button onClick={(e) => { e.stopPropagation(); setShowReportModal(true); setShowOptions(false); }} className="w-full flex items-center px-4 py-2.5 text-[14px] text-gray-700 hover:bg-gray-50 transition-colors"><Flag className="w-4 h-4 mr-3 text-gray-400" /> {t('thread.opt_report')}</button>
                    <button onClick={(e) => { e.stopPropagation(); setShowHistoryModal(true); setShowOptions(false); }} className="w-full flex items-center px-4 py-2.5 text-[14px] text-gray-700 hover:bg-gray-50 transition-colors"><History className="w-4 h-4 mr-3 text-gray-400" /> {t('thread.opt_history')}</button>
                    {amIAdmin && (
                      <><div className="h-px bg-gray-100 my-1"></div><button onClick={(e) => { e.stopPropagation(); handleDelete(); }} className="w-full flex items-center px-4 py-2.5 text-[14px] text-red-600 hover:bg-red-50 font-medium transition-colors"><Trash2 className="w-4 h-4 mr-3 text-red-500" /> {t('thread.opt_delete_admin')}</button></>
                    )}
                  </>
                )}
              </div>
      ...
      <div className={`flex items-center gap-6 pt-3 ${isNested ? 'mt-1' : 'border-t border-gray-100'}`}>
        ...
        {/* 👇 ĐÃ CẬP NHẬT THÊM SỐ LƯỢT CHIA SẺ VÀO ĐÂY 👇 */}
        <button onClick={handleShare} className="flex items-center gap-1.5 text-gray-500 hover:text-[#007AFF] font-medium transition-colors">
          <Send className="w-5 h-5 transition-transform active:scale-75 -rotate-12" />
          <span className="text-sm">{thread.shareCount || 0}</span>
        </button>
      </div>
            ...
            <div className="flex flex-col relative border-l-[2px] border-gray-100 ml-4 pl-3">
              {comments.map((comment: any) => (
                <div key={comment._id} className="relative z-10 mb-1">
                  ...
                  <Thread thread={comment} isNested={true} />
                </div>
              ))}
            </div>
          )}
      ...
      {showHistoryModal && (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in" onClick={(e) => { e.stopPropagation(); setShowHistoryModal(false); }}>
          <div className="bg-white rounded-2xl w-full max-w-[450px] p-6 shadow-2xl animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4"><h2 className="text-[18px] font-extrabold text-gray-900">{t('thread.edit_history')}</h2><button onClick={() => setShowHistoryModal(false)} className="text-gray-400 hover:text-gray-800"><X className="w-5 h-5"/></button></div>
            <div className="space-y-4 max-h-[50vh] overflow-y-auto pr-2 hidden-scrollbar">
               {(!thread.editHistory || thread.editHistory.length === 0) ? (
                 ...
                 [...thread.editHistory].reverse().map((history: any, idx: number) => (
                   <div key={idx} className="bg-[#f2f3f5] p-3 rounded-xl border border-gray-100"><p className="text-xs text-gray-500 font-bold mb-1.5">{new Date(history.editedAt).toLocaleString('vi-VN', { hour: '2-digit', minute: '2-digit', day: '2-digit', month: '2-digit', year: 'numeric' })}</p><p className="text-sm text-gray-800">{history.content}</p></div>
                 ))
               )}
            </div>
          </div>
      ...
      {showReportModal && (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in" onClick={(e) => { e.stopPropagation(); setShowReportModal(false); }}>
          <div className="bg-white rounded-2xl w-full max-w-[450px] p-6 shadow-2xl animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            <h2 className="text-[20px] font-extrabold text-gray-900 mb-1">{t('report.title')}</h2><p className="text-sm text-gray-500 mb-5 border-b border-gray-100 pb-4">{t('alerts.report_thanks')}</p>
            <div className="space-y-3 mb-6 max-h-[40vh] overflow-y-auto pr-2 hidden-scrollbar">
              {['reason_spam', 'reason_harassment', 'reason_inappropriate', 'reason_wrong_channel', 'reason_minors', 'reason_adult', 'reason_other'].map(r => (
                <label key={r} className="flex items-center gap-3 cursor-pointer p-3 rounded-xl border border-gray-100 hover:bg-gray-50 transition-colors"><input type="radio" name="report_reason" value={r} checked={reportReason === r} onChange={(e) => setReportReason(e.target.value)} className="w-4 h-4 text-[#5865F2] cursor-pointer" /><span className="text-[14px] font-medium text-gray-800">{t(`report.${r}`)}</span></label>
              ))}
