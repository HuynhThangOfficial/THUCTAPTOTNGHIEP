...
// BẢNG QUY TẮC NÂNG CẤP CHÍNH XÁC NHƯ YÊU CẦU CỦA BẠN
const LEVEL_REQUIREMENTS = [
  ...
  { level: 10, totalStones: 33, channels: 80, costToNext: 0 } // MAX LEVEL
];
...
export default function UpgradeServerModal({ workspace, channelCount, onClose }: Props) {
  ...
  const handleDonate = async () => {
    if ((currentUser.stones || 0) < 1) {
      alert(t('boost.not_enough_stones'));
      return;
    }
    setIsProcessing(true);
    try {
      await boostServer({ serverId: workspace._id as Id<"servers"> });
      alert(t('alerts.donate_success'));
    } catch (error: any) {
  ...
  const handleBuyStone = async () => {
    if (window.confirm(`${t('alerts.buy_confirm_title')}\n${t('alerts.buy_confirm_desc')}`)) {
      setIsProcessing(true);
      try {
        await buyStones({});
        alert(t('alerts.buy_success'));
      } catch (error: any) {
        alert(t('common.error') + ": " + error.message);
      }
      setIsProcessing(false);
        ...
        <div className="flex-1 overflow-y-auto hidden-scrollbar p-5 space-y-6">
          ...
          <div className="bg-[#1b5e20] rounded-2xl p-5 text-white shadow-md relative overflow-hidden">
            <div className="absolute top-0 right-0 opacity-10">
              <Diamond className="w-32 h-32 -mt-4 -mr-4" />
            </div>
          ...
          <div className="bg-white rounded-2xl p-4 border border-gray-200 shadow-sm flex flex-col items-center">
            ...
            <button onClick={handleBuyStone} disabled={isProcessing} className="w-full bg-white border-2 border-[#4ade80] text-[#16a34a] font-bold py-2.5 rounded-xl hover:bg-[#f0fdf4] transition-colors">
              {isProcessing ? '...' : t('upgrade.btn_buy')}
            </button>
          </div>

          {/* 👇 ĐÃ THAY ĐỔI: In Toàn Bộ Lộ Trình Lên Giao Diện 👇 */}
          <div>
                  ...
                  <div key={lvl.level} className={`flex items-center justify-between p-3 border-b border-gray-100 last:border-0 ${isAchieved ? 'bg-[#f0fdf4]' : ''}`}>
                    <div>
                      ...
                      <div className="text-[12px] font-medium text-gray-500 flex items-center mt-0.5">
                        ...
                        {/* Hiển thị số đá yêu cầu (tổng số) */}
