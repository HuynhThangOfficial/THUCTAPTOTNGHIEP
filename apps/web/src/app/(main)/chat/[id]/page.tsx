...
import { Send, Image as ImageIcon, MoreVertical, Pin, CheckCheck, Reply, Copy, Trash2, Smile, X, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function ChatPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#f2f3f5] relative overflow-hidden">
      
      {/* --- HEADER CHAT --- */}
      <div className="h-16 border-b border-gray-200 bg-white flex items-center justify-between px-6 shrink-0 shadow-sm z-20">
              ...
              <div className={`flex gap-3 group relative ${isMe ? 'flex-row-reverse' : 'flex-row'} items-end ${hasReactions ? 'mb-4' : (isLastInGroup ? 'mb-2' : 'mb-0.5')}`}>
                ...
                <div className={`flex flex-col max-w-[65%] ${isMe ? 'items-end' : 'items-start'} relative`}>
                  ...
                  <div className="flex items-center relative">
                    {/* Action Menu Hover (Web Specific) */}
                    {!msg.isDeleted && (
                      <div className={`hidden group-hover:flex items-center gap-1 absolute top-1/2 -translate-y-1/2 bg-white border border-gray-200 shadow-sm rounded-lg px-1 py-0.5 z-10 ${isMe ? 'right-full mr-2' : 'left-full ml-2'}`}>
                        ...
                        {/* Bảng Emoji bật ra khi bấm Smile */}
